...
public class ADM extends AbstractClassifier implements MultiClassClassifier,
                                                                        CapabilitiesHandler{
    @Override
    public String getPurposeString() {
        return "An Adaptive Learning System for Anomaly Detection in SDN by Ferdous et. al. (2025).";
    }
...
//            "moa.classifiers.meta.WekaRF");
        ...
        "choose an algorithm.",
        new String[]{"RF", "SysFor", "HT","CSF"},
        new String[]{"RF", "SysFor", "HT","CSF"}, 0);
        ...
        private int method;
        private boolean isCalculateConfusionMatrix=false;
        @Override
    ...
    public void learnFromBatchDataset(String batchFiles,int classIndex, String []options, String padding)
    {
       ...
       String header="Batch, Precision, Recall, F-Score, Accuracy, Time";       
...
//                adfl.displayAllForests();
                ...
                if(f.equals(""))
                {
                    ...
                    PRF=forest.forestRecallPrecisionFScore(testFile,classIndex);
                }                
                long []trainTime=adfl.getExeTime();
                float []trainAccuracy=adfl.getAccuracy();
                time=adfl.getTotalTime();
                batchStatus=PRF[1]+","+PRF[0]+", "
                            +PRF[2]+","+accuracy+", "
                            +time;
//                NumberFormat formatter = new DecimalFormat("#0.000");  
                String acc="\n"+bFile[i][1]+", "+batchStatus;
...
//        private boolean isCalculateConfusionMatrix=true;
        Forest()
        {
        ...
        {
            return confusionMatrix[0]+","+confusionMatrix[1]+","+confusionMatrix[2]+","+confusionMatrix[3];
        }
...
//           System.out.println("\n"+treeStr+"\n");
        ...
        public boolean isCorrectyClassified(Instance inst, int ci)
        {
            ...
            if(isCalculateConfusionMatrix){
              if(record[ci].equals("negative")&&pcv.equals("negative"))
              {
                  confusionMatrix[0]++;
              }
              else if(record[ci].equals("negative")&& pcv.equals("positive"))
              {
                  confusionMatrix[1]++;
              }
              else if(record[ci].equals("positive")&& pcv.equals("negative"))
              {
                  confusionMatrix[2]++;
              }
              else if(record[ci].equals("positive")&& pcv.equals("positive"))
              {
                  confusionMatrix[3]++;
              }
            }
        ...
        public float forestAccuracy(String testDataFile, int ci)
        {
            ...
            if(numberSamples>0)
                accuracy = (float) numberSamplesCorrect/ (float) numberSamples;
           return accuracy;
        }
        
        public float []forestRecallPrecisionFScore(String testDataFile, int ci)
        {
            
            float []PRF=new float[3];//0->recall, 1->precision, 2->f-score
            PRF[0]=0.0f;PRF[1]=0.0f;PRF[2]=0.0f;
            ArffFileStream testData=new ArffFileStream(testDataFile,ci);
            int cIndex=testData.getHeader().classIndex();
            
            int numberSamples =0;
//            int count = testData.getHeader().numInstances();
//            String []TrueLabels=new String [count];
//            String []PredictedLabels=new String [count];
            ...
            while(testData.hasMoreInstances())
            {
               ...
               String s=inst.toString();
               String []record=s.split(",");
               String trueL=record[cIndex];
               TrueLabels.add(trueL);
               if(!domainValues.contains(trueL))
               {
                    domainValues.add(trueL);
               }
               String preL=forestPrediction(record);
               PredictedLabels.add(preL);
               if(!domainValues.contains(preL))
               {
                    domainValues.add(preL);
               } 
               numberSamples++;
            }
            
            int numCVs=domainValues.size();
            if(numberSamples>0&&numCVs>0)
            {
                int [][]TPFPFN=new int[numCVs][3];//each row for each class, col0->TP, col1->FP, col2->FN
                for(int i=0;i<numCVs;i++)
                    for(int j=0;j<3;j++) 
                        TPFPFN[i][j]=0;
                for(int i=0;i<numberSamples;i++)   
                {
                    String trueL=TrueLabels.get(i);
                    String preL=PredictedLabels.get(i);
                    int trueIndex=domainValues.indexOf(trueL);
                    int preIndex=domainValues.indexOf(preL);
                    if(trueL.equals(preL))
                    {
                        TPFPFN[trueIndex][0]++;
                    }
                    else{
                ...
                for(int i=0;i<numCVs;i++)  
                {
                    if(TPFPFN[i][0]>0||TPFPFN[i][2]>0)
                    tr=tr+((float)TPFPFN[i][0]/((float)TPFPFN[i][0]+(float)TPFPFN[i][2]));
                    if(TPFPFN[i][0]>0||TPFPFN[i][1]>0)
                    tp=tp+((float)TPFPFN[i][0]/((float)TPFPFN[i][0]+(float)TPFPFN[i][1]));
                }
                PRF[0]=tr/(float)numCVs;
                PRF[1]=tp/(float)numCVs;
                if(PRF[0]>0||PRF[1]>0)
                    PRF[2]=(2.0f*PRF[0]*PRF[1])/(PRF[0]+PRF[1]);
            }
           return PRF;
        }
...
//               tdist[i]=0;
   ...
   private static class ForestFunctions {
    ...
    private static String buildWekaForest(int method,ArffFileStream trainArffs, int numTrees,int leafSize)
    {
        ...
        {
            wekaLearnerOption= new WEKAClassOption("baseLearner", 'T',
                "Classifier to train.", weka.classifiers.Classifier.class, "weka.classifiers.trees.CSForest -- -L 10 -N 10 -G 0.2 -S 0.3 -C 0.25 -cost-matrix \"[0.0 5.0; 1.0 1.0]\"");
        }
        else{
        ...
        try{
            ...
            else if(classifierName.equals("weka.classifiers.trees.SysFor")||classifierName.equals("weka.classifiers.trees.CSForest"))
            {
